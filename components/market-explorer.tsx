    ...
    <div className="mx-auto w-full max-w-6xl px-4 pb-32">
          ...
          <div className="flex items-center gap-1.5 rounded-full p-0.5 glass">
            ...
            {REGIONS.map((r) => (
              <button
                ...
                style={region === r ? { background: "var(--gold)", color: "#ffffff" } : { color: "var(--text-tertiary)" }}
              >
                {r}
              </button>
            ))}
        ...
        {RANGES.map((r) => (
          <button
            ...
            onClick={() => setRange(r)}
                ...
                style={range === r ? { background: "var(--gold)", color: "#ffffff" } : { border: "1px solid var(--line)", color: "var(--text-tertiary)" }}
          >
      ...
      <div className="mt-6" id="ai-analysis">
        <BentoCard className="!p-0">
          <AiAnalysis
            symbol={symbol}
            triggerRef={analysisTrigger}
            indicators={indicators}
            isDerivative={isDerivative}
            hasDerivLive={hasDerivLive}
          />
        </BentoCard>
      </div>
...
const QuoteHeader = memo(function QuoteHeader({
  ...
  const hasDerivLive = !!quote?.derivativeInfo?.hasLiveData
    ...
    <BentoCard className="relative overflow-hidden p-6 sm:p-8">
      ...
      <div className="relative flex flex-wrap items-end justify-between gap-6">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-col">
              <h1 className="font-heading text-3xl tracking-tight" style={{ color: "var(--text-primary)" }}>
                {quote?.name || displaySymbol(symbol)}
              </h1>
            ...
            {isDerivative && (
              <span className={`chip ${hasDerivLive ? "chip-pos" : ""}`} style={!hasDerivLive ? { background: "var(--gold-glow)", color: "var(--gold)" } : undefined}>
                {hasDerivLive ? "LIVE DERIVATIVE" : "CONTRACT MAPPED"}
              </span>
            )}
            <span
          ...
          {isDerivative && deriv ? (
            <div className="mt-2 text-xs" style={{ color: "var(--text-secondary)" }}>
              ...
              <strong className="font-mono text-[var(--text-primary)]">
                {deriv.underlyingName} ({deriv.underlyingSymbol}) @ {ccySym}{deriv.underlyingPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </strong>
              {deriv.underlyingChangePercent != null && (
                <span className={`ml-1.5 font-mono ${deriv.underlyingChangePercent >= 0 ? "text-[var(--pos)]" : "text-[var(--neg)]"}`}>
                  ({deriv.underlyingChangePercent >= 0 ? "+" : ""}{deriv.underlyingChangePercent.toFixed(2)}%)
                </span>
              )}
            </div>
          ) : (
        ...
        <div className="text-right">
          {isDerivative && !hasDerivLive ? (
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Option Premium (LTP)</div>
              <div className="stat-number mt-0.5 font-mono text-2xl font-bold tracking-tight" style={{ color: "var(--text-tertiary)" }}>
                —
              </div>
              <div className="mt-1 flex items-center justify-end gap-1 text-[11px] font-medium text-amber-500">
                <span>Live option data unavailable</span>
              </div>
            </div>
          ) : isDerivative && hasDerivLive ? (
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-wider text-emerald-500">Option Premium (LTP)</div>
              <div className="stat-number mt-0.5" style={{ color: "var(--text-primary)" }}>
                <AnimatedPrice value={quote?.price} ccySym={ccySym} />
              </div>
