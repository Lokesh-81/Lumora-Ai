          ...
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <div className="relative">
              <div className="flex items-center gap-1.5 rounded-xl px-2.5 py-1.5" style={{ border: "1px solid var(--line)", background: "var(--surface-alt)" }}>
                ...
                />
                {symbol && query && (
                  <button onClick={() => { setQuery(""); setOpen(false) }} className="p-0.5" suppressHydrationWarning><X className="h-3.5 w-3.5" style={{ color: "var(--text-tertiary)" }} /></button>
                )}
              </div>
              {open && (query.trim() || hits.length > 0) && (
                <div className="absolute left-0 z-50 mt-1.5 w-64 overflow-hidden rounded-xl" style={{ background: "var(--bg-elevated)", border: "1px solid var(--line-strong)" }}>
                  {hits.length === 0 ? (
                    ...
                    <button key={h.symbol} onClick={() => { setQuery(""); setSymbol(h.symbol); setOpen(false) }} className="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-[var(--panel-2)]" suppressHydrationWarning>
            ...
            <div className="flex flex-wrap gap-1.5">
              {DEFAULT_SYMBOLS.map((s) => (
                <button key={s} onClick={() => setSymbol(s)} suppressHydrationWarning className={`rounded-full px-2.5 py-1 text-[11px] font-mono transition-all duration-300 hover:scale-105 ${symbol === s ? "bg-[var(--gold-glow)] text-[var(--gold)]" : "text-[var(--text-tertiary)] hover:bg-[var(--panel-2)]"}`}>
                  {s}
                </button>
              ))}
            </div>
