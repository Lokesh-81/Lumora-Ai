...
import { generateAnalysis, getAiErrorDiagnostic, DISCLAIMER, AiConfigError, AiBillingError, AiOverloadedError } from "@/lib/ai/provider"
...
export async function POST(req: Request) {
  ...
  if (symbol.length > 100) return NextResponse.json({ error: "Invalid symbol" }, { status: 400 })
      ...
      async start(controller) {
        try {
          ...
          const errorMessage = err instanceof AiBillingError
            ? "AI analysis quota is temporarily exceeded. Please try again in a few moments."
            : err instanceof AiConfigError
            ? "AI service is not configured. Please contact support."
            : err instanceof AiOverloadedError
            ? err.message
            : err instanceof Error
            ? (err.message.startsWith("{") ? "AI analysis is temporarily unavailable. Please try again." : err.message)
