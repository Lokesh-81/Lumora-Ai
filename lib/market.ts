...
import { parseOptionQuery, suggestOptionContracts, getValidExpiries } from "@/lib/instrument"
...
export function resolveSymbol(input: string): string {
  ...
  if (optParsed?.underlyingSymbol) return optParsed.underlyingSymbol
  return raw
}
...
export async function getQuote(symbol: string, opts?: { withFundamentals?: boolean }): Promise<Quote | null> {
  ...
  if (optParsed && optParsed.underlyingSymbol && optParsed.strike && optParsed.optionType) {
    const underlyingQuote = await getQuote(optParsed.underlyingSymbol, { withFundamentals: false })
    if (underlyingQuote) {
      const validExpiries = getValidExpiries(optParsed.underlyingSymbol)
      const expiryDate =
        optParsed.expiry && /^\d{4}-\d{2}-\d{2}$/.test(optParsed.expiry)
          ? optParsed.expiry
          : (validExpiries[0]?.date ?? new Date().toISOString().slice(0, 10))

      const inst = createExchangeInstrument(
        optParsed.underlyingSymbol,
        optParsed.strike,
        optParsed.optionType,
        expiryDate
      )

      // Attempt to retrieve real live derivative market data from registered broker feeds
      const derivQuote = await derivativeManager.getDerivativeQuote(inst)
      const hasLive = !!(derivQuote && derivQuote.isLive && derivQuote.ltp != null)

      const ltp = hasLive ? (derivQuote!.ltp ?? 0) : 0
      const prevClose = hasLive ? (derivQuote!.close ?? 0) : 0
      const change = hasLive ? (derivQuote!.change ?? 0) : 0
      const changePercent = hasLive ? (derivQuote!.changePercent ?? 0) : 0

      return {
        symbol: optParsed.symbol,
        name: optParsed.name,
        // CRITICAL RULE: The underlying spot price must NEVER be substituted as the option's LTP.
        // If live derivative feed is unavailable, price is 0 / null.
        price: ltp,
        change,
        changePercent,
        currency: underlyingQuote.currency,
        exchange: optParsed.segment === "BFO" ? "BSE (BFO)" : optParsed.segment === "NFO" ? "NSE (NFO)" : underlyingQuote.exchange,
        marketState: underlyingQuote.marketState,
        previousClose: prevClose,
        open: hasLive ? (derivQuote!.open ?? undefined) : undefined,
        dayHigh: hasLive ? (derivQuote!.high ?? undefined) : undefined,
        dayLow: hasLive ? (derivQuote!.low ?? undefined) : undefined,
        volume: hasLive ? (derivQuote!.volume ?? undefined) : undefined,
        fiftyTwoWeekHigh: undefined,
        fiftyTwoWeekLow: undefined,
        assetType: "OPTION",
        timezone: underlyingQuote.timezone,
        updatedAt: Date.now(),
        derivativeInfo: {
          isDerivative: true,
          segment: optParsed.segment ?? "NFO",
          tradingsymbol: inst.tradingsymbol,
          canonicalSymbol: inst.canonicalSymbol,
          underlyingSymbol: optParsed.underlyingSymbol,
          underlyingName: optParsed.underlying ?? underlyingQuote.name,
          ...
          expiry: expiryDate,
          hasLiveData: hasLive,
          statusMessage: hasLive
            ? `Live derivative quote streaming via ${derivQuote!.sourceProvider}`
            : "Live option data unavailable",
          premium: hasLive ? (derivQuote!.ltp ?? undefined) : undefined,
          openInterest: hasLive ? (derivQuote!.openInterest ?? undefined) : undefined,
          openInterestChange: hasLive ? (derivQuote!.openInterestChange ?? undefined) : undefined,
          iv: hasLive ? (derivQuote!.iv ?? undefined) : undefined,
          delta: hasLive ? (derivQuote!.delta ?? undefined) : undefined,
          gamma: hasLive ? (derivQuote!.gamma ?? undefined) : undefined,
          theta: hasLive ? (derivQuote!.theta ?? undefined) : undefined,
          vega: hasLive ? (derivQuote!.vega ?? undefined) : undefined,
          bid: hasLive ? (derivQuote!.bid ?? undefined) : undefined,
          ask: hasLive ? (derivQuote!.ask ?? undefined) : undefined,
          bidQty: hasLive ? (derivQuote!.bidQty ?? undefined) : undefined,
          askQty: hasLive ? (derivQuote!.askQty ?? undefined) : undefined,
...
export async function getChart(symbol: string, range = "1mo", interval = "1d"): Promise<Candle[]> {
  ...
  const resolved = resolveSymbol(targetSymbol)
